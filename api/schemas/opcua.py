...
class StatusResponse(BaseModel):
    ...
    sampling: Optional[dict] = Field(
        ...
        description=(
            ...
            "- `publishing_period_s`: cada cuánto publica el servidor el lote "
            "(latencia máxima con la que retiene un cambio).\n"
            "- `delay_s`: ventana de seguridad con la que se emiten las "
            "muestras. Es cuánto tarda una muestra en VERSE; no desplaza el "
            "eje de tiempo, cada muestra se coloca en el instante en que el "
            "PLC la tomó.\n"
            "- `late_notifications`: notificaciones que llegaron con su ciclo "
            "ya emitido. Debe ser 0.\n"
            "- `recent_notifications`: últimas notificaciones crudas "
            "(variable, valor, timestamp del servidor, ciclo asignado, "
            "`grid_offset` y latencia de llegada), para diagnosticar el eje "
            "de tiempo.\n\n"
